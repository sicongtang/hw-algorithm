...
public class GraphAlgorithms {
	 ...
	 * v. If v is unreachable from u, or if u equals v, an empty path is
	 * returned.
	 *
	 * @param g
	 *            Graph instance
	 * @param u
	 *            Vertex beginning the path
	 * @param v
	 *            Vertex ending the path
	 * @param forest
	 *            must be a map that resulting from a previous call to DFS
	 *            started at u.
	 */
		...
		while (!level.isEmpty()) {
			...
			for (Vertex<V> u : level)
				for (Edge<E> e : g.outgoingEdges(u)) {
					...
					if (!known.contains(v)) {
						known.add(v);
						forest.put(v, e); // e is the tree edge that discovered
											// v
						nextLevel.addLast(v); // v will be further considered in
												// next pass
					}
				}
	...
	/**
	 * Converts graph g into its transitive closure. This uses the
	 ...
	 * order. If graph g has a cycle, the result will be incomplete.
	 */
	public static <V, E> PositionalList<Vertex<V>> topologicalSort(Graph<V, E> g) {
		...
		while (!ready.isEmpty()) {
			...
			topo.addLast(u);
			for (Edge<E> e : g.outgoingEdges(u)) { // consider all outgoing
													// neighbors of u
				Vertex<V> v = g.opposite(u, e);
				inCount.put(v, inCount.get(v) - 1); // v has one less constraint
													// without u
				if (inCount.get(v) == 0)
					ready.push(v);
			}
	...
	/**
	 * Computes shortest-path distances from src vertex to all reachable
	 * vertices of g.
	 *
	...
	public static <V> Map<Vertex<V>, Integer> shortestPathLengths(Graph<V, Integer> g, Vertex<V> src) {
		...
		for (Vertex<V> v : g.vertices()) {
			...
			pqTokens.put(v, pq.insert(d.get(v), v)); // save entry for future
														// updates
		}
			...
			for (Edge<Integer> e : g.outgoingEdges(u)) {
				...
				if (cloud.get(v) == null) {
					...
					if (d.get(u) + wgt < d.get(v)) { // better path to v?
						...
						pq.replaceKey(pqTokens.get(v), d.get(v)); // update the
																	// pq entry
					}
				}
			}
	...
	/**
	 * Reconstructs a shortest-path tree rooted at vertex s, given distance map
	 * d. The tree is represented as a map from each reachable vertex v (other
	 * than s) to the edge e = (u,v) that is used to reach v from its parent u
	 * in the tree.
	 */
	public static <V> Map<Vertex<V>, Edge<Integer>> spTree(Graph<V, Integer> g, Vertex<V> s, Map<Vertex<V>, Integer> d) {
		...
		for (Vertex<V> v : d.keySet())
			if (v != s)
				for (Edge<Integer> e : g.incomingEdges(v)) { // consider
																// INCOMING
																// edges
